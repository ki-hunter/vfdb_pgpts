...
def input_manager():
    """\
    After user confirmation that they wish to start automatic job submission
    and entry of the genus and MAX_JOB_AMOUNT this function wraps a function
    call of watchdog_boss() in a .sh file which is executed inside a screen.
    ...
    if yes:
        ...
        if genus not in jobs_content:
            print(jobs_content)
            print(f"Genus {genus} not found, typo?")
            input_manager()
            sys.exit()

        question = "How many jobs should be submitted at a time?"

        MAX_JOBS_AMOUNT = int(input(question))
...
def watchdog_boss(genus: str, MAX_JOBS_AMOUNT: int):
    """\
    This function finds all undone jobs for the genus, divides the total
    amount of jobs by the maximum amount of jobs to be run at a time
    (MAX_JOBS_AMOUNT) and then submits them in batches of size MAX_JOBS_AMOUNT.
    Completion of a job is indicated by a feedback file being created; the
    presence of which is checked for by watch_dog() every ten minutes.
    If for some reason the feedback files don't get created the active
    queue is checked.
    After all jobs in a batch are completed, the next batch is submitted.
...
#     """\
#     This function checks for the presence of the feedback files for a given
#     list of jobs
