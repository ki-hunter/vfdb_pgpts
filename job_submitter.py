...
def input_manager():
    ...
    function_call = f"watchdog_boss(MAX_JOBS_AMOUNT=5)"
...
def watchdog_boss(MAX_JOBS_AMOUNT:int):
    confirmation = "y"
    ...
    if yes:
        ...
        for job in job_list:
            ...
            while((not watchdog(job, genus_batches_dir)) or (not watchdog(job, jobs_dir))):
